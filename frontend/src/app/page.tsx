...
import { FileText, Cpu, Layers, Sparkles, AlertTriangle } from "lucide-react";
...
export default function Dashboard() {
  ...
  const renderContent = () => {
    if (activeView === "historial") {
      return (
        <div className="flex flex-col items-center justify-center h-[60vh] space-y-4 animate-in fade-in slide-in-from-bottom-8">
          <div className="p-6 rounded-full bg-white/5 border border-white/10">
            <Layers className="w-16 h-16 text-neon-blue opacity-50" />
          </div>
          <h2 className="text-3xl font-bold text-white tracking-tight">Historial de Análisis</h2>
          <p className="text-gray-400 max-w-md text-center">
            Aún no has procesado documentos reales con la IA. Todos tus archivos y hallazgos futuros se guardarán aquí de forma segura.
          </p>
        </div>
      );
    }
    
    if (activeView === "accesibilidad") {
      return (
        <div className="flex flex-col items-center justify-center h-[60vh] space-y-4 animate-in fade-in slide-in-from-bottom-8">
          <div className="p-6 rounded-full bg-white/5 border border-white/10">
            <Sparkles className="w-16 h-16 text-neon-gold opacity-50" />
          </div>
          <h2 className="text-3xl font-bold text-white tracking-tight">Preferencias de Accesibilidad</h2>
          <p className="text-gray-400 max-w-md text-center">
            Pronto podrás ajustar el tamaño de fuente, contraste, dictado por voz y modo de lectura inmersiva para el motor RAG.
          </p>
        </div>
      );
    }
    ...
    if (activeView === "configuracion") {
      return (
        <div className="flex flex-col items-center justify-center h-[60vh] space-y-4 animate-in fade-in slide-in-from-bottom-8">
          ...
          <h2 className="text-3xl font-bold text-white tracking-tight">Configuración del Sistema</h2>
          <p className="text-gray-400 max-w-md text-center">
            Las opciones de configuración avanzada (API Keys, Motor LLM, Límites de Tokens) estarán disponibles en el panel de administrador.
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 lg:p-10 relative">
        ...
        {/* Floating Credits */}
        <footer className="mt-20 py-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between text-[10px] font-bold tracking-widest gap-4 italic relative z-10">
